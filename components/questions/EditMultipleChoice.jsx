	...
	return (
		<>
			<CardContent className="mb-5">
				{options.map((option, index) => {
					return (
						<Box className="flex mx-10 my-3">
              <Checkbox className="px-5"/> 
							<Typography className="flex-grow">
                {option}
							</Typography>
							<DeleteOutlinedIcon className="text-black hover:text-red-400" onClick={() => deleteOption(index) } />
						</Box>
					);
				})}
			</CardContent>
        ...
        <Typography className="py-2 px-4 align-middle">
          ...
          className="flex-grow"
				...
				></Input>
				<IconButton className="mx-2 bg-slate-200/70 hover:bg-slate-200" onClick={() => addOption(newopt)}>
					<Add />
				</IconButton>
			</Box>
		</>
